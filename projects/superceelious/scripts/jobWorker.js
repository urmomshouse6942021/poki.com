...
(self.JobHandlers["ProcessImageData"] = function (a) {
  const b = a["buffer"],
    ...
    d = a["width"],
    e = a["height"];
  return a["flipY"] && FlipImageData(c, d, e), a["unpremultiply"] && UnpremultiplyImageData(c), { result: b, transferables: [b] };
}),
  self.addEventListener("message", (a) => {
    const b = a.data,
      c = b["type"];
    return "init" === c
      ? ((self.workerNumber = b["number"]), (self.dispatchPort = b["dispatch-port"]), (self.dispatchPort.onmessage = OnDispatchWorkerMessage), void (self.outputPort = b["output-port"]))
      : "terminate" === c
      ? void self.close()
...
function SendError(a, b) {
  a || self.outputPort.postMessage({ type: "error", jobId: self.activeJobId, error: b.toString() }), SendDone();
}
...
function OnDispatchWorkerMessage(a) {
  ...
  if ("_testMessageChannel" === c) return void self.outputPort.postMessage({ type: "_testMessageChannelOk" });
